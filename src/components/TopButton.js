...
const TopButton = forwardRef(({scrollTopPos}, ref) => {
    const [isVisible, setIsVisible] = useState(false);
  
    useEffect(() => {
        let heightToShow = 500;
        setIsVisible(scrollTopPos > heightToShow);
    }, [scrollTopPos]);
  ...
  return (
    <div className={`sticky w-full h-20 md:h-40 bottom-4 z-20 flex justify-center transition duration-300 ease-in-out ${isVisible ? 'translate-y-0' : 'translate-y-24'}`}>
        <div className={`w-12 h-12 md:w-16 md:h-16 rounded-full animate-bounce hover:cursor-pointer`} onClick={() => handleClick(ref.scrollRef)}>
